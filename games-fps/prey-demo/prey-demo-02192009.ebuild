...
src_install() {
	...
	doins data/prey_demo_readme.txt data/prey_demo_license.txt \
		data/punkbuster_license.txt || die "doins texts failed"
	doins -r data/punkbuster-linux-x86/pb || die "doins punkbuster failed"
	...
	doexe data/prey-demo-linux-x86/libgcc_s.so.1 \
		...
		data/prey-demo-linux-x86/libstdc++.so.5 || die "doexe executables failed"

	exeinto "${dir}/base"
	doexe data/prey-demo-linux-x86/base/gamex86.so \
		|| die "doexe base/gamex86.so failed"

	insinto "${dir}/base"
	doins -r data/prey-demo-linux-data/base/* || die "doins data failed"
