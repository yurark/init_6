...
inherit base autotools multilib multilib-minimal flag-o-matic \
	python-any-r1 toolchain-funcs versionator ${GIT_ECLASS}
...
if [[ $(get_version_component_range 3 ${MY_PV}) = 0 ]] ; then
    FOLDER="$(get_version_component_range 1-2 ${MY_PV})"
else
    FOLDER="${PV}"
fi
...
src_prepare() {
	...
	epatch "${FILESDIR}"/mesa-10.0-nv50-fix-build.patch
	epatch "${FILESDIR}"/mesa-9.2-hardware-float.patch
	epatch "${FILESDIR}"/mesa-9.2-evergreen-big-endian.patch

	# https://bugs.freedesktop.org/show_bug.cgi?id=73512
	use opencl && epatch "${FILESDIR}"/0001-opencl-use-versioned-.so-in-mesa.icd.patch
